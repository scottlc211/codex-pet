...
- 透明、无边框、置顶桌面窗口
- 支持本地 `.png`、`.jpg`、`.jpeg`、`.webp`、`.gif`、`.svg`、`.apng` 宠物资源
...
- 支持按主题为 13 种桌宠状态独立选择、预览和重置动作
- 工作任务支持最多 3 个并发、独立状态、取消、超时重试、历史记录和任务终端定位
...
设置 > 主题         切换主题，或卸载由 Codex Pet 导入的主题
设置 > 提醒         添加、编辑、删除每周或指定日期的一次性提醒
设置 > 工作任务     并行发送任务，点击任务卡打开对应项目终端
```
...
主题卡片上的卸载按钮只会删除 `~/.codex-pet/pets/` 中选中的主题；兼容扫描的 `~/.codex/pets/` 资源不会被应用删除。
...
任务正文通过标准输入传递，不参与 shell 命令拼接；同一时间最多运行 3 个由桌宠启动的 Codex 任务，超过后进入队列。
“设置 > 工作任务”和桌宠任务浮层会按项目分别显示思考、命令、编辑、等待、重试、取消及结束状态。任务卡会记录提交时选择的终端类型和项目目录：Windows Terminal 使用任务专属窗口标题，重复点击会优先聚焦已有窗口；其他终端会在对应项目目录新开窗口。

任务进程仍由桌宠后台托管，以便可靠捕获 JSONL 状态、取消、超时和重试；任务终端是该项目的可定位操作入口，不会冒充或接管用户自行启动的外部终端窗口。
