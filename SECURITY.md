...
RustSec 会报告 Tauri Linux 依赖链中的 GTK3/UNIC 组件已停止维护，以及 `glib 0.18.5` 的 `VariantStrIter` 未定义行为（RUSTSEC-2024-0429）。当前 Windows 发布链不使用这些 GTK3 组件；Linux 构建仍需跟随 Tauri 上游迁移后消除这些警告。
